(()=>{const content=document.getElementById('sd9000Content');const searchBox=document.querySelector('#sd9000Search input');const getPairs=(store)=>{const list=[];for(let i=0;i<store.length;i++){const k=store.key(i);const v=store.getItem(k);list.push({key:k,val:v})}
return list};const renderList=(list,type)=>{content.innerHTML='';const filter=(searchBox.value||'').toLowerCase();list.filter(item=>item.key.toLowerCase().includes(filter)||item.val.toLowerCase().includes(filter)).forEach(({key,val})=>{const row=document.createElement('div');row.className='sd9000Row';row.innerHTML=`
      ...
      })()">🔓 Decode JWT</button></div>
      <div style="margin-top:10px;"><button onclick="alert('👽 You found the terminal: type "open_terminal()" in console')">🧪 Hidden Terminal</button></div>
    `};window.sd9000Render=async(type='localStorage')=>{if(type==='localStorage')renderList(getPairs(localStorage),'localStorage');else if(type==='sessionStorage')renderList(getPairs(sessionStorage),'sessionStorage');else if(type==='Cookies')renderCookies();else if(type==='IndexedDB')await renderIndexedDB();else if(type==='Extras')renderExtras();};searchBox.addEventListener('input',()=>window.sd9000Render(window.sd9000CurrentType));document.getElementById('sd9000AddBtn').onclick=()=>{const k=document.getElementById('sd9000NewKey').value;const v=document.getElementById('sd9000NewVal').value;if(!k)return;const target=window.sd9000CurrentType==='localStorage'?localStorage:window.sd9000CurrentType==='sessionStorage'?sessionStorage:null;if(target){target.setItem(k,v);window.sd9000Render(window.sd9000CurrentType)}else if(window.sd9000CurrentType==='Cookies'){document.cookie=`${encodeURIComponent(k)}=${encodeURIComponent(v)}; path=/`;window.sd9000Render('Cookies')}
...
if(e.storageArea===sessionStorage&&window.sd9000CurrentType==='sessionStorage'){window.sd9000Render('sessionStorage')}});window.open_terminal=()=>{const term=prompt("👾 Terminal activated. Command:");try{const result=eval(term);alert('✅ '+result)}catch(e){alert('❌ '+e.message)}};window.sd9000Render(window.sd9000CurrentType)})()
